...
function Layout({ children, page, setPage }) {
  ...
  // Apply dark class to <html> so Tailwind's dark: variant works correctly
  useEffect(() => {
    if (state.darkMode) {
      ...
      <div className="min-h-screen flex bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-white transition-colors">
        ...
        <aside className={`fixed top-0 left-0 h-full w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 shadow-xl z-40 flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
          ...
          <div className="px-5 py-4 border-t border-gray-200 dark:border-gray-800">
            <button onClick={() => dispatch({ type: "TOGGLE_DARK" })} className="w-full flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
              <span className="text-base">{state.darkMode ? "☀️" : "🌙"}</span>
              <span className="font-medium">{state.darkMode ? "Light Mode" : "Dark Mode"}</span>
              <div className={`ml-auto w-10 h-5 rounded-full transition-colors relative ${state.darkMode ? "bg-teal-600" : "bg-gray-300"}`}>
                <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${state.darkMode ? "translate-x-5" : "translate-x-0.5"}`} />
              </div>
            </button>
          </div>
        </aside>
